...
/*   Updated: 2024/02/09 16:37:17 by wsonepou      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void	images_loader(t_game *game)
{
	...
	if (txtr.floor == NULL || txtr.wall == NULL || txtr.player == NULL
		|| txtr.coin == NULL || txtr.exit == NULL || txtr.enemy == NULL)
	...
	game->img.t = mlx_texture_to_image(game->mlx, txtr.enemy);
	if (game->img.f == NULL || game->img.w == NULL
		|| game->img.p == NULL || game->img.c == NULL
		|| game->img.e == NULL || game->img.t == NULL)
